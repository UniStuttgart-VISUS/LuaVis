...
static constexpr float fpi = pi;
static constexpr float fpi2 = pi * 2.f;
}
...
{
	return angle * (math::fpi / 180.f);
}

inline float radToDeg(float angle)
{
	return angle * (180.f / math::fpi);
}
...
inline bool getBarycentricCoords(sf::Vector2f pos, sf::Vector2f t1, sf::Vector2f t2, sf::Vector2f t3, float & b1,
                                 float & b2, float & b3)
{
