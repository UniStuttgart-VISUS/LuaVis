...
Widget::~Widget()
{
	...
	myControllerCallbacks.clearCallbacks();
	myMouseCallbacks.clearCallbacks();
	myTouchCallbacks.clearCallbacks();
...
CallbackHandle<MouseEvent> Widget::addMouseCallback(CallbackFunction<MouseEvent> func, int typeFilter, int order)
{
	return myMouseCallbacks.addCallback(func, typeFilter, order);
}

CallbackHandle<TouchEvent> Widget::addTouchCallback(CallbackFunction<TouchEvent> func, int typeFilter, int order)
{
	return myTouchCallbacks.addCallback(func, typeFilter, order);
}

CallbackHandle<KeyEvent> Widget::addKeyboardCallback(CallbackFunction<KeyEvent> func, int typeFilter, int order)
{
	return myKeyCallbacks.addCallback(func, typeFilter, order);
}

CallbackHandle<ControllerEvent> Widget::addControllerCallback(CallbackFunction<ControllerEvent> func, int typeFilter,
                                                              int order)
{
	return myControllerCallbacks.addCallback(func, typeFilter, order);
}

CallbackHandle<StateEvent> Widget::addStateCallback(CallbackFunction<StateEvent> func, int typeFilter, int order)
{
	return myStateCallbacks.addCallback(func, typeFilter, order);
}

CallbackHandle<> Widget::addTickCallback(CallbackFunction<> func, int order)
{
...
void Widget::fireTouchEvent(TouchEvent event)
{
	myTouchCallbacks.fireCallback(event.type, event);
}
...
void Widget::fireControllerEvent(ControllerEvent event)
{
	myControllerCallbacks.fireCallback(event.type, event);
}
