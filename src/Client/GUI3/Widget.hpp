...
#include <Client/GUI3/Events/TouchEvent.hpp>
...
public:
	 ...
	 */
	CallbackHandle<MouseEvent> addMouseCallback(CallbackFunction<MouseEvent> func, int typeFilter, int order = 0);

	/**
	 * Adds a callback for mouse events with the specified filter and order (lower = earlier).
	 */
	CallbackHandle<TouchEvent> addTouchCallback(CallbackFunction<TouchEvent> func, int typeFilter, int order = 0);

	/**
	 * Adds a callback for keyboard events with the specified filter and order (lower = earlier).
	 */
	CallbackHandle<KeyEvent> addKeyboardCallback(CallbackFunction<KeyEvent> func, int typeFilter, int order = 0);

	/**
	 * Adds a callback for controller events with the specified filter and order (lower = earlier).
	 */
	CallbackHandle<ControllerEvent> addControllerCallback(CallbackFunction<ControllerEvent> func, int typeFilter,
	                                                      int order = 0);

	/**
	 * Adds a callback for state change events with the specified filter and order (lower = earlier).
	 */
	CallbackHandle<StateEvent> addStateCallback(CallbackFunction<StateEvent> func, int typeFilter, int order = 0);
	 ...
	 */
	CallbackHandle<> addTickCallback(CallbackFunction<> func, int order = 0);
	...
	/**
	 * Notifies the widget of a touch event.
	 */
	void fireTouchEvent(TouchEvent event);
	...
	/**
	 * Notifies the widget of a controller event.
	 */
	void fireControllerEvent(ControllerEvent event);
...
private:
	...
	CallbackManager<TouchEvent> myTouchCallbacks;
	CallbackManager<KeyEvent> myKeyCallbacks;
	CallbackManager<ControllerEvent> myControllerCallbacks;
