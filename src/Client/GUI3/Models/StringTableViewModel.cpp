...
void StringTableViewModel::setDataModel(Ptr<DataModel> dataModel)
{
	if (myDataModel != dataModel)
	{
		...
		if (dataModel)
		{
			myCellDataCallback = dataModel->addEventCallback(
			    [this](DataModel::Event event) {
			...
			myTableDataCallback = dataModel->addEventCallback(
			    [this](DataModel::Event event) {
				    updateTableData();
			...
			myColumnCountCallback = dataModel->addEventCallback(
			    [this](DataModel::Event event) {
				    updateColumnCount();
...
void StringTableViewModel::onParentChanged(Widget * oldParent)
{
	...
	if (getParentWidget())
	{
		myParentCallback = getParentWidget()->addStateCallback(
...
float StringTableViewModel::Row::getScaleFactor() const
{
	return (getSize().x - config().get(leftSpacing) - config().get(rightSpacing)
	        - ((int) myColumns.size() - 1) * config().get(betweenSpacing))
	       / getSize().x;
}
