...
bool Container::addWidget(Widget & widget)
{
	...
	data.callback = widget.addStateCallback(
	    [this, &widget](StateEvent event) {
...
void Container::handleStateEvent(StateEvent event)
{
	...
	case StateEvent::WindowClosed:
		// Forward resource, parent application, configuration change or window close to all widgets.
...
void Container::handleMouseEvent(MouseEvent event)
{
	...
	case MouseEvent::Leave:
	{
		...
		if (myMouseOverWidget != nullptr)
		{
			...
			myMouseOverWidget->fireMouseEvent(MouseEvent::generatePositionalEvent(
			    MouseEvent::Leave, myMouseOverWidget->getInverseTransform().transformPoint(event.position)));
	...
	case MouseEvent::ScrollY:
	{
		...
		if (event.type == MouseEvent::ButtonUp)
		{
			...
			if (mouseDownWidget != nullptr)
			{
				...
				mouseDownWidget->fireMouseEvent(MouseEvent::generateButtonEvent(
				    MouseEvent::ButtonUp, mouseDownWidget->getInverseTransform().transformPoint(mousePos),
				    event.button));
				...
				{
					mouseDownWidget->fireMouseEvent(MouseEvent::generateButtonEvent(
					    MouseEvent::Click, mouseDownWidget->getInverseTransform().transformPoint(mousePos),
					    event.button));
				}
		...
		else
		{
			...
			if (isMouseOverWidgetChanged)
			{
				...
				if (myMouseOverWidget != nullptr)
				{
					// Leave old widget.
					myMouseOverWidget->fireMouseEvent(MouseEvent::generatePositionalEvent(
					    MouseEvent::Leave, myMouseOverWidget->getInverseTransform().transformPoint(mousePos)));
				}
			...
			if (event.type == MouseEvent::Move)
			{
				...
				for (Widget * mouseDownWidget : myMouseDownWidgets)
				{
					...
					{
						// Forward mouse event.
						mouseDownWidget->fireMouseEvent(MouseEvent::generateRepositionedEvent(
						    event, mouseDownWidget->getInverseTransform().transformPoint(mousePos)));
					}
				}
			}
		}
...
void Container::handleTouchEvent(TouchEvent event)
{
	// Forward touch event to locally focused widget.
	// TODO: add proper position-based forwarding for touch events
	if (getFocusedWidget() != nullptr)
	{
		TouchEvent transformedEvent(event, getFocusedWidget()->getInverseTransform().transformPoint(event.position));
		getFocusedWidget()->fireTouchEvent(transformedEvent);
	}
}
...
void Container::handleControllerEvent(ControllerEvent event)
{
	// Forward controller event to locally focused widget.
	if (getFocusedWidget() != nullptr)
	{
		getFocusedWidget()->fireControllerEvent(event);
	}
}
