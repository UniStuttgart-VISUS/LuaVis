	...
	myIsTitleBarDown(false)
{
	...
	myCloseButton.addMouseCallback(
	...
	myMaximizeButton.addMouseCallback(
	    [this](MouseEvent event) {
...
void Window::Titlebar::updateWindowParent()
{
	...
	else
	{
		...
		myParentMouseCallback = myWindow->getParent()->addMouseCallback(
		    [this](MouseEvent event) {
...
void Window::ResizeOverlay::handleMouseEvent(MouseEvent event)
{
	...
	case MouseEvent::Move:
		...
		else if (!myRecursionLock)
		{
			...
			sf::Vector2f windowPositionDiff =
			    transformVector(myWindow->getInverseTransform(), myInitialWindowPosition - myWindow->getPosition());
			...
			sf::Vector2f windowPositionChange =
			    transformVector(myWindow->getTransform(), windowSizeChange * shiftFactor);
...
void Window::updateMaximizedSize()
{
	if (isMaximized() && getParent() != nullptr)
	{
		if (isProportional())
		{
			sf::Vector2f limits = convertFrameSizeToContentSize(getParent()->getSize());
			sf::FloatRect fitRect = priv::fitAspectRatio(sf::Vector2f(getAspectRatio(), 1), sf::FloatRect({}, limits));
			sf::Vector2f fitRectSize = convertContentSizeToFrameSize(sf::Vector2f(fitRect.width, fitRect.height));
			setRect(fitRect.left, fitRect.top, fitRectSize.x, fitRectSize.y);
		}
		else
		{
...
void Window::updateProportionalSize()
{
	if (isProportional() && !isMaximized())
	{
