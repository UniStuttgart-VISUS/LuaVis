...
void Table::setViewModel(Ptr<TableViewModel> model)
{
	if (myModel != model)
	{
		...
		if (model != nullptr)
		{
			...
			model->addEventCallback(
			    [this](TableViewModel::Event event) {
...
void Table::showRow(std::size_t row)
{
	...
	data.mouseCallback = data.widget->addMouseCallback(
	    [this, row](MouseEvent event) {
