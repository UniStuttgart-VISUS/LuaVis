...
--
-- Adapted for NecroNecro by Marukyu
--

local m = {}

-- Fix size of number at 8 bytes, instead of determining it dynamically
...
function m.packWithPrefix (data, prefix)
    local buffer = {prefix}
    packers[type(data)](buffer, data)
    return tconcat(buffer)
end
...
local function cursor_prefixed_string (str, prefix)
...
function m.unpackWithPrefix (s, prefix)
    checktype('unpack', 1, s, 'string')
    local cursor = cursor_prefixed_string(s, prefix)
    if cursor == nil then
        return false
    end
    local data = unpack_cursor(cursor)
