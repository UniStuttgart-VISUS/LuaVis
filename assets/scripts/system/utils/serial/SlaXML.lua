...
(modified for NecroNecro by Marukyu)
--]=====================================================================]
...
function slaxml.parse(callbacks, xml, options)
	...
	local function finishText()
		if first>textStart and callbacks.text then
			...
			if text then callbacks.text(unescape(text)) end
		end
	end
	...
	local function findPI()
		...
		if first then
			finishText()
			if callbacks.pi then callbacks.pi(match1,match2) end
			pos = last+1
			textStart = pos
			return true
		end
	...
	local function findComment()
		...
		if first then
			finishText()
			if callbacks.comment then callbacks.comment(match1) end
			pos = last+1
			textStart = pos
			return true
		end
	...
	local function findCDATA()
		...
		if first then
			finishText()
			if callbacks.text then callbacks.text(match1) end
			pos = last+1
			textStart = pos
			return true
		end
	...
	local function closeElement()
		...
		if first then
			...
			if callbacks.startElement then callbacks.startElement(unpack(currentElement)) end
			if callbacks.attribute then
				for i=1,currentAttributeCt do
					...
					callbacks.attribute(unpack(currentAttributes[i]))
				end
			end

			if match1=="/" then
				pop(nsStack)
				if callbacks.closeElement then callbacks.closeElement(unpack(currentElement)) end
			end
			return true
		end
	end
	...
	local function findElementClose()
		...
		if first then
			finishText()
			if callbacks.closeElement then callbacks.closeElement(match1,nsURI) end
			pos = last+1
			textStart = pos
			pop(nsStack)
			return true
...
function slaxml.parser(callbacks)
	return {
		parse = function (xml, options)
			return slaxml.parse(callbacks or defaultCallbacks, xml, options)
		end
	}
end
